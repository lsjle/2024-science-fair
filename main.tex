...
\end{table}
 ...
 介紹
 P-Tuning v2
 原理:P-tuning v2 利用深度提示調整，對Transformer的每一層輸入應用連續提示。帶來更多可學習的參數，同時更深層結構中的提示能給預訓練模型帶來更直接的影響。
 LoRA
 原理:先疆域訓練模型的權重凍結，再新增AB兩部分，A使用高斯分布填滿，而B使用0填滿，接著分別調整AB兩部分。最後的模型權重和AB有關。
 Adapter Tuning
 原理:
