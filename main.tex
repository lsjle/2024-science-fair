...
\backgroundsetup{contents=Public Review Draft} 
...
\begin{wrapfigure}{r}{0.3\textwidth}
	\centering
	\includegraphics[width=0.3\textwidth]{v1}
...
\begin{wrapfigure}{r}{0.3\textwidth}
	\centering
	\includegraphics[width=0.3\textwidth]{v2}
...
\begin{wrapfigure}{r}{0.3\textwidth}
	\centering
	\includegraphics[width=0.3\textwidth]{v3}
...
\end{wrapfigure}
v1 data size small fineetune cuz a bigger effect 
	\CJKfakebold{思考}：有可能第二版
...
%	此模型在圖靈測驗中表現經整理成混淆矩陣後\footnote{轉換為百分比表示}（下表），其準確率可以高達、F1 score可以高達。
	...
	\subsection{二、不同模型比對}
	第一版模型訓練後表現較第三版模型好，可能是因為我們我們本次實驗所提供的資料集相對於模型原有的資料過少，而對原本模型造成混淆的現象，但因第一版的資料量較少所以較不容易造成混淆，而第三版中因為模型原有資料量過大所以導致我們混淆了原本的模型而使其表現較差。
%	\subsection{二、和人類表現比對}
	...
	\section{陸、結論}
		\begin{enumerate}
			\item 我們透過訓練三個不同版本的模型，觀察到第一版模型訓練後表現較第三版模型好
