...
df=pd.read_csv("scorenew.csv")[8:14]
...
plt.axhline(y=dfc['BLEU-4'][7],linestyle=":",c="#1f77b4")
plt.axhline(y=dfc[' Rouge1'][7],linestyle=":",c="#ff7f0e")
plt.axhline(y=dfc[' Rouge2'][7],linestyle=":",c="#2ca02c")
plt.axhline(y=dfc['RougeL'][7],linestyle=":",c="#d62728")
plt.legend(["BLEU-4", "Rouge1", "Rouge2","RougeL"])

plt.ylabel("分數(來源見研究方法)")
plt.xlabel("檢查點(500-3000)")
plt.title("ChatGLM2不同檢查點TrustfulQA表現比較")
# plt.axis([0, 12,])
plt.show()


plt.savefig('../../chart/2vc.eps', format='eps')
plt.savefig('../../chart/2vcp.png', format='png')